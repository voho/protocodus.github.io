...
export function createVoyage({ canvas, sectionIds, onTelemetry, onTooSlow }) {
  ...
  // If any builder below throws, the stage must not survive it: its resize
  // and visibility listeners would keep a half-built scene alive — and
  // renderable — behind the classic page the failure falls back to.
  try {
    return assemble();
  } catch (err) {
    stage.dispose();
  ...
  function assemble() {
    ...
    // A few frames now, so ENGAGE lifts the curtain on a finished scene
    // rather than a compile hitch — the first render is where WebGL pays for
    // all its shaders at once — and then the stage holds. The boot dialog is
    // opaque; rendering behind it would spend the visitor's battery on
    // frames nobody can see.
    let engaged = false;
    let warmFrames = 0;
    stage.add(null, () => {
      if (!engaged && ++warmFrames >= 3) stage.hold();
    });
    stage.play();
    ...
    return {
      ...
      engage(playFlyby = true) {
        engaged = true;
        stage.release();
        if (playFlyby) flyby.play();
        plan.measure();
      },
