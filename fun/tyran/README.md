...
Real is tuned against this fixed sample for ten successful attempts out of 100; the [calibration history](tests/balance/calibration-history.json) records the search. These are calibration results, not an independent validation sample or an estimate of human win rates. The headless benchmark omits ground scenery, its collisions, destruction rewards and ground bonus drops. Ground interactions and full browser gameplay are checked separately.

Startup uses a centered, accessible loading overlay until assets and initial terrain are prepared. Terrain strips retain their substrate composition; only a flat seam clear remains per frame. Ground vignettes and building fixtures skip transparent image regions, rain and dust share a stroke submission, and debris/wrecks share camera-state saves. Resolution, 60 Hz simulation, interpolated movement, animation cadence and effect counts remain unchanged.
