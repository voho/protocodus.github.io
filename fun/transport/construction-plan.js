import { build, constructionCost, tileAt } from './model.js';
...
export function quoteBuildPlan(game, tool, points, options) {
  const placements = uniquePoints(points).map(({ x, y }) => {
    const resolved = resolveBuildTool(game, tool, x, y, options);
    return { x, y, tool: resolved, cost: constructionCost(game, resolved, x, y) };
  });
  return { placements, cost: placements.reduce((sum, placement) => sum + placement.cost, 0) };
}
...
export function buildPlan(game, tool, points, options) {
  ...
  const message = count ? `${tool === 'bulldoze' ? 'Cleared' : 'Built'} ${count} tile${count === 1 ? '' : 's'} · ${money}${failed ? ` · ${failed} skipped. ${errorText}` : ''}`
