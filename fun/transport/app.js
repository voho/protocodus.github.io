import { getVehiclePurchase, getVehicleUpgrade, getFleetUpgrade, upgradeRouteVehicle, upgradeFleet, constructionCost, priceFor, inflationInfo, createGame, build, buildPath, addRoute, removeRoute, tick, saveGame, loadGame, BIOMES, INDUSTRIES, CARGO, BUILD_COSTS, stationCoverage, WORLD_SIZES, industryConditions, settlementSuitability, weatherAt } from './model.js';
...
const TOOL_INFO = {
 ...
 road:{name:'Road',icon:'road',key:'R',detail:'Drag to build roads for buses and trucks.'},
 rail:{name:'Railway',icon:'rail',key:'T',detail:'Drag to lay track for trains.'},
...
let pointer = null, spaceDown = false, sounds = false, audioContext;
...
mobileToggle.addEventListener('click', () => { const open = $('.sidebar').classList.toggle('mobile-open'); mobileToggle.classList.toggle('open',open); mobileToggle.setAttribute('aria-expanded',String(open)); mobileToggle.innerHTML=icon(open?'close':'road')+'<span>Manage</span>'; });
...
function setTool(next) { cancelRoutePicking(); tool=next; preview=[]; selected=null; $('#inspector').hidden=true; canvas.classList.toggle('build-mode',tool!=='inspect'); const detail=toolDescription(tool); $('#status-message').textContent=detail; renderPanel(); if(window.innerWidth<=700)closeMobile(); }
function setView(next) { if(next!=='routes')cancelRoutePicking(); view=next; $$('[data-mobile-view]').forEach(el=>el.classList.toggle('active',el.dataset.mobileView===next)); $$('.nav-button[data-view]').forEach(el=>{el.classList.toggle('active',el.dataset.view===view);el.setAttribute('aria-current',el.dataset.view===view?'page':'false');}); renderPanel(); if(window.innerWidth<=700 && !$('.sidebar').classList.contains('mobile-open'))mobileToggle.click(); }
function toolCard(key) { const info=TOOL_INFO[key]; return `<button class="tool-card ${tool===key?'active':''}" data-tool="${key}" aria-pressed="${tool===key}" title="${escapeHTML(info.detail)}">${icon(info.icon)}<span class="tool-title">${info.name}</span><span class="tool-cost">${compactMoney(priceFor(game,BUILD_COSTS[key]))}${lineTools.has(key)?' / tile':''}</span>${info.key?`<span class="shortcut">${info.key}</span>`:''}</button>`; }
...
function buildPanel() {
 const groups={network:['road','rail','bridge','railbridge','tunnel','railtunnel','bus-stop','train-stop','port','bulldoze'],towns:['residential','commercial','industrial','city']};
 ...
 return `<div class="panel-heading"><h2>Build</h2></div>${tabs}${category==='industry'?industries:`<div class="tool-grid">${groups[category].map(toolCard).join('')}</div>`}<div class="tool-description">${escapeHTML(toolDescription(tool))}</div><div class="build-bottom-tools"><button class="compact-tool ${tool==='inspect'?'active':''}" data-tool="inspect">${icon('inspect')} Explore <span>Esc</span></button>${category!=='network'?`<button class="compact-tool danger ${tool==='bulldoze'?'active':''}" data-tool="bulldoze">${icon('bulldoze')} Bulldozer <span>X</span></button>`:''}</div>${category==='towns'?buildingPalette():`<button class="text-button" data-action="help">How to play <span>↗</span></button>`}`;
}
...
function cancelRoutePicking() {
 ...
 if(wasPicking)$('#status-message').textContent=toolDescription(tool);
}
...
function beginRoutePicking(key) {
 setTool('inspect');routePicking=key;renderPanel();showRoutePickHint();closeMobile();canvas.focus({preventScroll:true});
}
...
function townsPanel() { return `<div class="panel-heading"><h2>Towns</h2><span>${game.cities.length}</span></div>${game.cities.map(city=>`<button class="entity-card" data-city="${city.id}"><h3>${escapeHTML(city.name)}${icon('arrowup')}</h3><p>${cargoBadge('passengers',{count:Math.floor(city.population)})} <span>residents</span></p><div class="entity-metric"><span>Transport</span><span>${city.activity>0?'Connected':'No service'}</span></div></button>`).join('')}<div class="section-divider"></div><button class="button button-primary full" data-tool="city">${icon('city')} Found town · ${compactMoney(priceFor(game,BUILD_COSTS.city))}</button><button class="text-button" data-action="development">Zone a neighborhood <span>↗</span></button>`; }
function industryPanel() { return `<div class="panel-heading"><h2>Industries</h2><span>${game.industries.length} sites</span></div>${game.industries.map(i=>{const def=INDUSTRIES[i.kind];return `<button class="entity-card" data-industry="${i.id}"><h3>${escapeHTML(i.name||def.name)}${icon('arrowup')}</h3>${cargoRecipe(def.inputs,def.outputs)}<div class="entity-metric"><span>${integer(Object.values(i.inventory||{}).reduce((a,b)=>a+b,0))} stored</span><span>${Math.round((i.capacity||1)*100)}% capacity</span></div></button>`;}).join('')}<div class="section-divider"></div><button class="button button-primary full" data-action="industry-build">${icon('factory')} Build industry</button><button class="text-button" data-action="chains">Production chains <span>↗</span></button>`; }
function renderPanel() {
 ...
 panel.querySelectorAll('[data-category]').forEach(el=>el.addEventListener('click',()=>{category=el.dataset.category;renderPanel();}));
 ...
 panel.querySelectorAll('[data-city]').forEach(el=>el.addEventListener('click',()=>{const city=game.cities.find(c=>String(c.id)===el.dataset.city);renderer.focus(city.x,city.y);inspect(city.x,city.y,'city');closeMobile();}));
 panel.querySelectorAll('[data-industry]').forEach(el=>el.addEventListener('click',()=>{const i=game.industries.find(c=>String(c.id)===el.dataset.industry);renderer.focus(i.x,i.y);inspect(i.x,i.y);closeMobile();}));
...
function updateHud() {
 ...
 const profit=(game.monthlyIncome||0)-(game.monthlyExpenses||0);$('#profit').textContent=(profit>=0?'+':'−')+hudMoney(profit);$('#profit').title=(profit>=0?'+':'−')+money(profit)+' / month';$('#profit').className=profit>=0?'positive':'negative';
 ...
 const activeStationIds=new Set(game.routes.filter(r=>r.active).flatMap(r=>r.stops));const served=game.cities.filter(c=>game.stations.some(s=>activeStationIds.has(s.id)&&Math.hypot(s.x-c.x,s.y-c.y)<=6)).length;
 ...
 if($('#zoom-label').textContent!==zoomLabel)$('#zoom-label').textContent=zoomLabel;
 ...
 $$('[data-route-status]').forEach(el=>{const r=game.routes.find(r=>String(r.id)===el.dataset.routeStatus);if(r){el.textContent=r.active?'RUNNING':'DISCONNECTED';el.classList.toggle('route-offline',!r.active);}});
 $$('[data-route-revenue]').forEach(el=>{const r=game.routes.find(r=>String(r.id)===el.dataset.routeRevenue);if(r)el.textContent=money(r.revenue);});
...
function inspect(x,y,kind='') {
 ...
 else if(industry){const d=INDUSTRIES[industry.kind],conditions=industryConditions(game,industry),typical=Object.values(d.outputs).reduce((a,b)=>a+b,0)*(industry.capacity||1)*conditions.productivity;title=industry.name||d.name;tag=`INDUSTRY · ${x}, ${y}`;body=`${cargoRecipe(d.inputs,d.outputs)}${industryDestinations(industry)}<div class="inspector-grid"><div><small>Capacity</small><strong>${Math.round((industry.capacity||1)*100)}%</strong></div><div><small>Typical / day</small><strong>${typical.toLocaleString('en-US',{maximumFractionDigits:1})}</strong></div></div>${localConditions(conditions)}<div class="section-divider"></div><div class="ledger">${Object.entries(industry.inventory||{}).map(([key,n])=>`<div class="ledger-row">${cargoBadge(key,{label:true})}<strong>${integer(n)}</strong></div>`).join('')||'<span class="micro-note">Storage empty</span>'}</div><p>Add a stop within 5 tiles.</p>`;}
 else if(tile.building&&BUILDINGS[tile.building.kind]){const b=BUILDINGS[tile.building.kind];title=b.name;tag=b.tier?b.tier.toUpperCase()+' HOME':BUILDING_GROUPS[b.group].name.toUpperCase();const nearest=game.cities.reduce((best,c)=>!best||Math.hypot(c.x-x,c.y-y)<Math.hypot(best.x-x,best.y-y)?c:best,null);body=`<div class="inspector-building"><canvas width="96" height="100" data-building-sprite="${tile.building.kind}" aria-hidden="true"></canvas><p>${escapeHTML(b.tier||BUILDING_GROUPS[b.group].name)} · ${nearest&&Math.hypot(nearest.x-x,nearest.y-y)<=10?escapeHTML(nearest.name):'Countryside'}</p></div><div class="inspector-grid"><div><small>Collection</small><strong>${escapeHTML(BUILDING_GROUPS[b.group].name)}</strong></div><div><small>Development</small><strong>Level ${tile.building.level||1}</strong></div></div>`;}
 else if(city&&!tile.zone){title=city.name;tag='TOWN';body=`<div class="inspector-grid"><div><small>Population</small><strong>${integer(city.population)}</strong></div><div><small>Activity</small><strong>${integer(city.activity||0)}</strong></div></div>${localConditions(settlementSuitability(game,city))}<button class="button button-primary full" id="zone-town">${icon('house')} Add zones</button>`;}
 ...
 if(tile.zone)body+=localConditions(settlementSuitability(game,{x,y},tile.zone));
...
function openModal(html) {
 layersView?.close();cancelRoutePicking();
 if(pointer&&canvas.hasPointerCapture(pointer.id))canvas.releasePointerCapture(pointer.id);
 pointer=null;preview=[];spaceDown=false;canvas.classList.remove('dragging');$('#placement-tip').hidden=true;
...
function activateGame(next) {
 cancelRoutePicking();
 if(pointer&&canvas.hasPointerCapture(pointer.id))canvas.releasePointerCapture(pointer.id);
 game=next;pointer=null;spaceDown=false;selected=null;hover=null;preview=[];tool='inspect';
 view='build';category='network';buildingGroup='homes';chainSelection={};
 formDraft={name:'',mode:'road',from:'',to:'',cargo:'passengers'};
 routeFilters={query:'',mode:'all',status:'all',cargo:'all'};
...
function openWorld() {
 let chosen=game.biome,chosenSize='huge';
 ...
 $('#generate-world').addEventListener('click',()=>{
  ...
  activateGame(createGame({biome:chosen,seed,size:chosenSize}));persist();modalPreviousSpeed=1;closeModal();
...
function openAtlas() {
 ...
 atlas.addEventListener('click',e=>{const rect=atlas.getBoundingClientRect();renderer.focus((e.clientX-rect.left)/rect.width*game.width,(e.clientY-rect.top)/rect.height*game.height);closeModal();closeMobile();});
...
function openHelp(tab='basics') {
 ...
 const basics=`<div class="guide-grid"><div class="guide-item"><span>${icon('road')}Build a network</span><p>Drag roads or rail. Bridge water; tunnel through mountains. Place ports beside riverbanks and coasts.</p></div><div class="guide-item"><span>${icon('route')}Connect two stops</span><p>Place stops or ports within 5 tiles of customers. Connect them, choose cargo, then launch a bus, train or ship.</p></div><div class="guide-item"><span>${icon('factory')}Supply factories</span><p>Deliver every input in a recipe. Towns buy finished goods. Freight returns empty; passengers travel both ways.</p></div><div class="guide-item"><span>${icon('leaf')}Slow, local growth</span><p>Roads and deliveries attract development. Nature, services and weather shape how each neighborhood grows.</p></div></div><div class="keyboard-help"><span><kbd>R</kbd> Road</span><span><kbd>T</kbd> Rail</span><span><kbd>S</kbd> Stop</span><span><kbd>P</kbd> Port</span><span><kbd>B</kbd> Bridge</span><span><kbd>X</kbd> Bulldozer</span><span><kbd>G</kbd> Grid</span><span><kbd>L</kbd> Layers</span><span><kbd>H</kbd> Home</span><span><kbd>M</kbd> Map</span><span><kbd>Space</kbd> Pause / hold to pan</span><span><kbd>Esc</kbd> Explore</span></div>`;
...
function paintPath(points) { const result=points.length>1?buildPath(game,tool,points):build(game,tool,points[0].x,points[0].y);toast(result.message,!result.ok);preview=[];updateHud();if(result.ok){persist();if(view!=='build')renderPanel();} }
...
canvas.addEventListener('pointerdown',e=>{
 ...
 pointer={id:e.pointerId,x:e.clientX,y:e.clientY,lastX:e.clientX,lastY:e.clientY,start:tile,moved:false,pan:tool==='inspect'||e.button!==0||spaceDown};canvas.setPointerCapture(e.pointerId);
...
canvas.addEventListener('pointermove',e=>{
 ...
 if(pointer&&pointer.id===e.pointerId){const dx=e.clientX-pointer.lastX,dy=e.clientY-pointer.lastY;if(Math.hypot(e.clientX-pointer.x,e.clientY-pointer.y)>5)pointer.moved=true;if(pointer.pan){renderer.pan(dx,dy);}else if(lineTools.has(tool)){preview=gridLine(pointer.start,hover);}pointer.lastX=e.clientX;pointer.lastY=e.clientY;}
 const tip=$('#placement-tip');if(tool!=='inspect'&&tileAt(hover.x,hover.y)&&!pointer?.pan){const points=preview.length?preview:[hover],n=points.length,cost=points.reduce((sum,p)=>sum+constructionCost(game,tool,p.x,p.y),0);tip.textContent=`${TOOL_INFO[tool]?.name||BUILDINGS[tool]?.name||INDUSTRIES[tool]?.name} · ${money(cost)}${n>1?' · '+n+' tiles':''}`;const rect=canvas.getBoundingClientRect();tip.style.left=Math.max(4,Math.min(rect.width-220,e.clientX-rect.left+17))+'px';tip.style.top=Math.max(4,Math.min(rect.height-55,e.clientY-rect.top+18))+'px';tip.hidden=false;}else tip.hidden=true;
});
canvas.addEventListener('pointerup',e=>{if(!pointer||pointer.id!==e.pointerId)return;const p=pointer;pointer=null;canvas.classList.remove('dragging');if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);if(!p.moved&&e.button===0&&!spaceDown&&pickRouteStopAt(p.start.x,p.start.y))return;if(p.pan){if(!p.moved&&e.button===0&&!spaceDown)inspect(p.start.x,p.start.y);}else{const points=preview.length?preview:[p.start];if(points.every(p=>tileAt(p.x,p.y)))paintPath(points);else{toast('Keep construction within the world boundary.',true);preview=[];}}});
canvas.addEventListener('pointercancel',()=>{pointer=null;preview=[];canvas.classList.remove('dragging');});
...
$('#zoom-in').onclick=()=>{renderer.zoomAt(1.2);updateHud();};$('#zoom-out').onclick=()=>{renderer.zoomAt(1/1.2);updateHud();};$('#home-view').onclick=()=>renderer.focus(game.cities[0].x,game.cities[0].y);
$$('[data-zoom-level]').forEach(el=>el.addEventListener('click',()=>{renderer.setZoom(Number(el.dataset.zoomLevel));updateHud();}));
...
document.addEventListener('keydown',e=>{
 ...
 if(e.code==='Space'){e.preventDefault();if(!e.repeat){spaceDown=true;spaceStarted=performance.now();}return;}
 if(e.repeat)return;const key=e.key.toLowerCase();
 if(key==='escape'){setTool('inspect');closeMobile();return;}
 const keys={r:'road',t:'rail',s:'bus-stop',p:'port',b:'bridge',x:'bulldoze','1':'residential','2':'commercial','3':'industrial'};
 if(keys[key]&&!e.metaKey&&!e.ctrlKey){category=['1','2','3'].includes(key)?'towns':'network';view='build';setView('build');setTool(keys[key]);}
 if(key==='l'&&!e.ctrlKey&&!e.metaKey&&!e.altKey){e.preventDefault();layersView?.toggle();}
...
document.addEventListener('keyup',e=>{if(e.code==='Space'){if(spaceDown&&!pointer&&performance.now()-spaceStarted<260)changeSpeed(speed===0?previousSpeed:0);spaceDown=false;}});
window.addEventListener('blur',()=>{spaceDown=false;pointer=null;preview=[];});
...
syncLayerControls();
updateRegion();renderPanel();updateHud();changeSpeed(1);renderer.resize();if(window.innerWidth<=700)renderer.focus(game.cities[0].x+2,game.cities[0].y);
// Establish the browser save immediately, including on the first visit.
persist();
function frame(now){const elapsed=Math.min((now-lastFrame)/1000,.15);lastFrame=now;if(!document.hidden&&speed>0)tick(game,elapsed*speed);renderer.render(now,{tool,hover,preview,selected,routeStops:routePickStops()});if(now-hudAt>400){updateHud();hudAt=now;const notice=game.notifications[0];if(notice&&notice.id!==lastNoticeId){lastNoticeId=notice.id;toast(notice.message,notice.type==='warning');}if(selected&&!$('#inspector').hidden&&!$('#inspector').contains(document.activeElement))inspect(selected.x,selected.y,selected.kind);}if(now-minimapAt>900||lastRevision!==game.revision){renderer.drawMinimap($('#minimap'));minimapAt=now;lastRevision=game.revision;}if(now-saveAt>20000&&!saveDialogController)persist();if(now-panelAt>7000&&!$('#route-form')?.contains(document.activeElement)&&(view==='industry'||view==='towns')){renderPanel();panelAt=now;}requestAnimationFrame(frame);}
