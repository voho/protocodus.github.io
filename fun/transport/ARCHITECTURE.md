...
`terrain-elevation.js` reads the precise saved `tile.elevation` on a 0–16 scale; water is level zero. Integer levels are reserved for construction and inspection, so natural slopes do not acquire artificial terraces. Rendering does not mutate terrain or add a second saved height field. A nonnegative cubic B-spline over neighboring tile centers blends heights, biome ground colors and geological coverage into a continuous surface. Analytical gradients provide soft northwest hill lighting; interpolation preserves continuous heights and slopes across tile boundaries without overshoot. A raised tile therefore affects its neighbors, including across chunk boundaries.

Ground is sampled at six points per tile into a reusable small canvas, then smoothly drawn into the existing bounded chunk cache. Sampling phases, soil moisture, fine grain and geological texture are world-aligned, producing byte-identical shared samples in overlapping chunks. Two-tile fingerprint padding covers the spline support, and fingerprints include fractional elevation. Forest density follows neighboring woodland and regional noise; regenerated rock and mountain patches cluster around geology with varied related silhouettes. The overview uses the same altitude colors and a cheaper slope estimate. `tests/terrain-elevation.test.mjs` verifies interpolation and seams; `tests/terrain-elevation-browser-check.mjs` covers live elevation edits across chunks, exact restoration, all environments, zooms and display densities.

`shoreline.js` traces water boundaries, filters neighboring contour controls and adds subtle world-aligned bank variation. The resulting curves replace repeated rounded tile corners while preserving land/water tile centers, isolated ponds and narrow channels. Padded chunks produce identical shared curves; `water-art.js` adds local shallows and damp banks without a uniform bright outline. `tests/shoreline.test.mjs` checks navigation clearance and seams. `tests/landscape-realism-browser-check.mjs` captures deterministic town, forest, highland and coast scenes across environments, zooms and display densities, and checks canopy clipping and cache limits in isolated browser storage.

## Terrain construction controls

`app.js` groups earthworks and explicit crossings in the expandable Network → Terrain & crossings panel. Raise/lower gestures use the same deduplicated construction plan, prices, notifications, revision handling and autosave path as other tools. Each drag applies one level to each selected tile, once. Bridge/tunnel gestures snap to one horizontal or vertical axis; the shared plan quote provides level, total price and validation feedback. Road/rail selection changes the explicit crossing mode without adding four permanent toolbar buttons.

Span validation and atomic construction belong to the model and construction planner, so previews and committed spans share the same geometry and constraints. The renderer uses the existing saved elevation for terrain and span metadata for level crossings. `tests/earthworks-browser-check.mjs` exercises the controls through browser pointer events and isolated local storage.

`terrain-engineering.js` centralizes earthwork protection, span geometry and network edge validation. Interior span tiles store `structureLevel` and `structureAxis`; approaches remain ordinary network tiles. Pathfinding permits entry along the span axis at its deck or bore level, while legacy untagged networks retain their original connection rules. Construction invalidates route caches once, demolition removes the metadata, and the existing sparse save codec preserves both elevation edits and span fields without a format migration. `structure-visibility.js` shares underground vehicle visibility between the renderer and night lighting. Model tests cover atomic failures, routing, repairs and saved-world restoration; the engineering renderer check verifies portals, underground vehicles, previews and cache limits at all three zooms.
