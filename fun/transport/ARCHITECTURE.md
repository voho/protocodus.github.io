...
`WORLD_SIZES` exports regional 128×96, large 256×192, huge 512×384 and the default vast 768×576. Vast has 64 towns and twelve industry clusters; `MAX_WORLD_TILES` bounds save decoding. Old 100×72 saves remain valid. The tile codec stores base properties in four bytes per tile, uses exact elevation/detail palettes, and stores buildings/zones/extra flags sparsely. Public generated roads are excluded from company road upkeep.
...
The cache budget starts at 48 MiB and can grow for large high-density viewports, up to 256 MiB. The minimap/atlas uses a low-resolution world image, never a full-resolution map raster. It reuses its pixel buffer across ecology revisions and paints sparse industries separately from terrain scans. The app coalesces continuous wheel gestures into a single neighboring view change, and all button and keyboard inputs use the same three-level contract.
