...
export const BUILDINGS = {
  ...
  barracks: {name:'Field barracks',cost:320,hp:1100,size:2,buildTime:15,power:-15,requires:['core'],description:'Trains rifle squads and fast recon rovers.',sight:8},
...
const cell=(x,y)=>Math.floor(y)*W+Math.floor(x);
...
function addEntity(s,team,kind,type,x,y,built=true){
  ...
  if(kind==='building'){e.queue=[];if(type==='refinery')e.haulerPending=true;s.navVersion++;}else if(type==='harvester'){e.cargo=0;e.harvestPhase='gather';e.order={type:'harvest'};}
...
function generateMap(s){
  ...
  for(let b=0;b<105;b++){
    ...
    for(let yy=Math.floor(y-r);yy<=y+r;yy++)for(let xx=Math.floor(x-r);xx<=x+r;xx++)if(inside(xx,yy)&&sq(xx-x)+sq(yy-y)<r*r&&random(s)>.13)s.terrain[yy*W+xx]=1;
  }
  const clear=(x,y,r)=>{for(let yy=Math.floor(y-r);yy<=y+r;yy++)for(let xx=Math.floor(x-r);xx<=x+r;xx++)if(inside(xx,yy)&&sq(xx-x)+sq(yy-y)<=r*r)s.terrain[yy*W+xx]=0;};
  clear(12,37,11);clear(59,12,11);
  // Three guaranteed routes prevent unlucky seeds from sealing either base.
  for(let i=0;i<=100;i++){const t=i/100;clear(12+47*t,37-25*t,2.6);clear(12+47*t,37-25*t+Math.sin(t*Math.PI)*10,2.1);clear(12+47*t,37-25*t-Math.sin(t*Math.PI)*10,2.1);}
  const patch=(x,y,amounts)=>{let k=0;for(let dy=-2;dy<=2;dy++)for(let dx=-2;dx<=2;dx++)if(dx*dx+dy*dy<=5){const xx=x+dx,yy=y+dy;if(inside(xx,yy)){s.terrain[yy*W+xx]=0;s.minerals[yy*W+xx]=amounts[k++];}}};
  for(const [x,y] of [[20,38],[12,44],[6,25],[29,27],[35,40]]){
    const amounts=Array.from({length:21},()=>320+Math.floor(random(s)*300));patch(x,y,amounts);patch(70-x,48-y,amounts);
  }
}

export function createGame(seed='ASH-001',difficulty='normal'){
  ...
  for(let team=0;team<2;team++){
    const building=(type,x,y)=>addEntity(s,team,'building',type,team?70-x-BUILDINGS[type].size:x,team?48-y-BUILDINGS[type].size:y);
    const unit=(type,x,y)=>addEntity(s,team,'unit',type,team?70-x:x,team?48-y:y);
...
function rebuildNavigation(s){
  ...
  for(let i=0;i<N;i++)s.blocked[i]=s.terrain[i]===1?1:0;
...
function walkable(s,x,y,r=.19){
  ...
  for(const yy of [y-r,y+r])for(const xx of [x-r,x+r])if(s.blocked[cell(xx,yy)])return false;
  return true;
}
function seen(s,team,e){if(e.team===team)return true;const c=center(e);if(!inside(c.x,c.y))return false;if(s.visible[team][cell(c.x,c.y)])return true;if(e.kind==='building')for(let y=e.y;y<e.y+e.size;y++)for(let x=e.x;x<e.x+e.size;x++)if(s.visible[team][y*W+x])return true;return false;}
...
export function canPlace(s,team,type,x,y){
  ...
  for(let yy=y;yy<y+d.size;yy++)for(let xx=x;xx<x+d.size;xx++){
    const i=yy*W+xx;if(!s.visible[team][i])return bad('Requires sensor coverage');if(s.blocked[i])return bad('Ground is obstructed');if(s.minerals[i]>0)return bad('Shard field obstructs construction');
  }
...
export function trainUnit(s,team,type){
  const d=UNITS[type];if(s.status!=='playing')return bad('Operation has ended');if(!d||![0,1].includes(team))return bad('Unknown unit');
  if(s.teams[team].credits<d.cost)return bad('Insufficient credits');
  const producer=own(s,team,d.producer).filter(e=>e.progress>=1).sort((a,b)=>a.queue.length-b.queue.length)[0];
  if(!producer)return bad(`Requires ${BUILDINGS[d.producer].name}`);
  if(d.requires.some(key=>!completed(s,team,key)))return bad('Technology unavailable');
  if(producer.queue.length>=6)return bad('Production queue full');
...
export function issueOrder(s,ids,order){
  ...
  units.forEach((u,i)=>{
    ...
    if(u.type==='harvester'&&u.cargo>=UNITS.harvester.capacity)u.harvestPhase='return';
  });
}
export function stopUnits(s,ids){for(const id of ids){const u=getEntity(s,id);if(u?.kind==='unit'){u.order={type:u.type==='harvester'?'harvest':'idle'};u.targetId=null;u.path=[];u.repath=0;}}}
...
function updateFog(s){
  ...
  for(const [id,m] of Object.entries(s.ai.known))if(s.visible[1][cell(m.x,m.y)]&&!s.entities.some(e=>e.id===Number(id)&&alive(e)&&seen(s,1,e)))delete s.ai.known[id];
}
...
function findPath(s,u,tx,ty,stop=0){
  const start=cell(u.x,u.y),goalX=clamp(Math.floor(tx),0,W-1),goalY=clamp(Math.floor(ty),0,H-1);
  ...
  while(heap.length&&count++<N){
    ...
    for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){
      if(!dx&&!dy)continue;const xx=x+dx,yy=y+dy;if(!inside(xx,yy))continue;
...
function navigate(s,u,tx,ty,dt,stop=.2){
  ...
  const p=u.path[0],dx=p.x-u.x,dy=p.y-u.y,d=Math.hypot(dx,dy),step=Math.min(d,UNITS[u.type].speed*dt);
  if(d<.08){u.path.shift();return false;}
  const nx=u.x+dx/d*step,ny=u.y+dy/d*step;u.angle=Math.atan2(dy,dx);
  if(walkable(s,nx,ny)){u.x=nx;u.y=ny;}else{u.repath=0;u.path=[];}
  if(step>=d-.02)u.path.shift();return false;
}

function nearestMineral(s,u,x=u.x,y=u.y){
  let best=-1,score=Infinity;const region=s.regions[cell(u.x,u.y)];
...
function harvest(s,u,dt){
  ...
  if(u.harvestPhase==='return'){
    ...
    if(navigate(s,u,c.x,c.y,dt,stop)||distance(u,c)<stop+.4){u.unload=(u.unload||0)+dt;if(u.unload>=1.2){s.teams[u.team].credits+=u.cargo*(depot.type==='core'?.6:1);u.cargo=0;u.unload=0;u.harvestPhase='gather';u.repath=0;}}
    return;
  }
  ...
  if(navigate(s,u,x,y,dt,.75)||Math.hypot(u.x-x,u.y-y)<1.1){const amount=Math.min(28*dt,s.minerals[u.mineralTile],cap-u.cargo);s.minerals[u.mineralTile]-=amount;u.cargo+=amount;}
}

function explore(s,u,dt){
  const order=u.order;
  ...
  if(navigate(s,u,order.x,order.y,dt,.35))order.tile=undefined;
}
...
function acquire(s,e,r){
  let best=null,score=Infinity;for(const enemy of s.entities){if(enemy.team===e.team||!alive(enemy)||!seen(s,e.team,enemy))continue;const d=targetDistance(e,enemy);if(d>r)continue;const threat=enemy.kind==='building'?(enemy.type==='turret'?-1:1):enemy.type==='harvester'?.8:0;const value=d+threat;if(value<score){score=value;best=enemy;}}
  return best;
}
function armorMultiplier(attacker,target){
  const armor=target.kind==='building'?'building':UNITS[target.type].armor;
  const table={rifle:{infantry:1,light:.55,heavy:.23,building:.4},scout:{infantry:1.25,light:.65,heavy:.26,building:.4},tank:{infantry:.65,light:1.1,heavy:1,building:1},artillery:{infantry:.9,light:1,heavy:.8,building:1.5},turret:{infantry:.75,light:1,heavy:1,building:.8}};
...
function hurt(s,target,amount,attacker){
  ...
  if(target.hp<=0){s.teams[attacker.team].kills++;const c=center(target);s.effects.push({type:'explosion',x:c.x,y:c.y,life:.6,maxLife:.6,team:target.team,size:target.kind==='building'?target.size:1});if(target.kind==='building'){s.navVersion++;event(s,`${BUILDINGS[target.type].name} destroyed`,target.team);}if(target.type==='core'){s.status=target.team===0?'defeat':'victory';event(s,target.team===0?'Command nexus lost. Operation failed.':'Hostile nexus destroyed. Sector secured.');}}
}
function shoot(s,e,target){
  const d=definition(e),a=center(e),b=center(target);e.angle=Math.atan2(b.y-a.y,b.x-a.x);e.cooldown=d.interval||1;
  s.effects.push({type:e.type==='artillery'?'shell':'shot',x:a.x,y:a.y,tx:b.x,ty:b.y,life:e.type==='artillery'?.35:.13,maxLife:e.type==='artillery'?.35:.13,team:e.team});
  hurt(s,target,d.damage*armorMultiplier(e,target),e);
  if(e.type==='artillery')for(const other of s.entities)if(other!==target&&other.team!==e.team&&alive(other)&&distance(center(other),b)<1.6)hurt(s,other,d.damage*.45*armorMultiplier(e,other),e);
}

function stepUnit(s,u,dt){
  u.repath-=dt;u.cooldown=Math.max(0,u.cooldown-dt);
  if(u.type==='harvester'&&!['move','explore'].includes(u.order.type)){if(u.order.type!=='harvest')u.order={type:'harvest'};harvest(s,u,dt);return;}
  const d=UNITS[u.type],order=u.order;
  if(order.type==='move'){
    const arrived=navigate(s,u,order.x,order.y,dt,.35);
  ...
  if(target){
    ...
    if(order.type==='attack'||order.type==='attackMove'){navigate(s,u,c.x,c.y,dt,d.range+(target.kind==='building'?target.size*.45:0)-.2);return;}
  }
  if(order.type==='explore'){explore(s,u,dt);return;}
  if(order.type==='attackMove'||order.type==='attack'){if(navigate(s,u,order.x,order.y,dt,.45))u.order={type:'idle'};}
}
...
function spawnAt(s,producer,type){
  ...
  if(type!=='harvester'&&producer.rally){u.order={type:'attackMove',x:producer.rally.x,y:producer.rally.y};}
...
function separateUnits(s,dt){
  ...
  for(let i=0;i<units.length;i++)for(let j=i+1;j<units.length;j++){
    const a=units[i],b=units[j];let dx=b.x-a.x,dy=b.y-a.y,d=Math.hypot(dx,dy);const min=(a.size+b.size)*.43;
    if(d>=min)continue;if(d<.001){dx=.01;dy=.004;d=Math.hypot(dx,dy);}
    const push=Math.min((min-d)*.45,dt*.9),px=dx/d*push,py=dy/d*push;
    if(walkable(s,a.x-px,a.y-py)){a.x-=px;a.y-=py;}if(walkable(s,b.x+px,b.y+py)){b.x+=px;b.y+=py;}
  }
}
...
function aiBuild(s,type){
  ...
  const preferred=type==='turret'?{x:c.x+toward.x*9,y:c.y+toward.y*9}:c;
  const candidates=[];
...
function thinkAI(s){
  ...
  if(!constructing){
    ...
    else if(own(s,1,'turret').length<(hard?3:2)&&s.time>75)aiBuild(s,'turret');
  ...
  const waypoints=[{x:35,y:28},{x:18,y:40},{x:12,y:15},{x:50,y:43},{x:9,y:48}];
  ...
  if(s.time>=ai.nextRaid&&fighting.length>=(easy?9:hard?5:7)){
    const priority={refinery:0,reactor:1,turret:3,core:2,barracks:2,factory:2};
...
function step(s,dt){
  ...
  const powers=[powerStats(s,0),powerStats(s,1)];
  for(const e of [...s.entities]){
    ...
    if(e.kind==='building'){
      ...
      if(e.type==='turret'){e.cooldown=Math.max(0,e.cooldown-dt*rate);const target=acquire(s,e,BUILDINGS.turret.range);e.targetId=target?.id??null;if(target&&e.cooldown<=0&&powers[e.team].ratio>=1)shoot(s,e,target);}
    }else stepUnit(s,e,dt);
  }
  separateUnits(s,dt);
