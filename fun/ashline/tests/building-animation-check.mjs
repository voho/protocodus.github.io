...
try {
  ...
  const checks = await page.evaluate(async () => {
    ...
    const results = [];
    const producedUnit = Object.fromEntries(Object.keys(BUILDINGS).map(type => [type, Object.keys(UNITS).find(u => UNITS[u].producer === type)]));
    for (const team of [0, 1]) for (const [type, d] of Object.entries(BUILDINGS)) {
      const e = { id: 3, kind: 'building', type, team, size: d.size, hp: d.hp, maxHp: d.hp, progress: 1, angle: .7, queue: producedUnit[type] ? [{ type: producedUnit[type], progress: .9 }] : [], processingAmount: buildingRole(type) === 'refinery' ? 120 : 0, processingTotal: buildingRole(type) === 'refinery' ? 200 : 0, research: buildingRole(type) === 'lab' ? { id: 'vehicleWeapons', progress: .4 } : undefined, reserve: buildingRole(type) === 'capacitor' ? 400 : undefined, lastShot: 1.1 };
      const first = pixels(e, 1.2), later = pixels({ ...e, research: e.research ? { ...e.research, progress: .7 } : undefined, reserve: e.reserve == null ? undefined : 900 }, 2.17), repeated = pixels(e, 1.2), lowPower = pixels(e, 1.2, .3);
      results.push({ type, role: buildingRole(type), team, nonempty: first.some((v, i) => i % 4 === 3 && v > 0), changed: first.some((v, i) => v !== later[i]), frozen: first.every((v, i) => v === repeated[i]),
        powerResponds: d.power >= 0 || first.some((v, i) => v !== lowPower[i]),
        idleActivity: pixels({ ...e, queue: [], research: undefined, processingAmount: 0, processingTotal: 0 }, 1.2).some(Boolean),
        queueOnlyProcessingActivity: buildingRole(type) === 'refinery' && pixels({ ...e, processingAmount: 0, processingTotal: 0 }, 1.2).some((v, i) => i % 4 === 3 && Math.floor(i / 4 / 192) < 105 && v > 0),
    ...
    const sheet = document.createElement('canvas'); sheet.id = 'building-sheet'; sheet.width = Object.keys(BUILDINGS).length * 160; sheet.height = 720;
    sheet.style.cssText = `position:fixed;inset:0;z-index:99999;width:${sheet.width}px;height:720px`; document.body.append(sheet);
    document.querySelectorAll('dialog[open]').forEach(dialog => dialog.close());
    const c = sheet.getContext('2d'); c.fillStyle = '#142027'; c.fillRect(0, 0, sheet.width, 720);
    for (const team of [0, 1]) Object.entries(BUILDINGS).forEach(([type, d], i) => {
      const column = 160, x = i * column, y = team * 360;
      ...
      const e = { id: i + 1, kind: 'building', type, team, size: d.size, hp: d.hp, maxHp: d.hp, progress: 1, angle: .7, queue: producedUnit[type] ? [{ type: producedUnit[type], progress: .9 }] : [], processingAmount: buildingRole(type) === 'refinery' ? 120 : 0, processingTotal: buildingRole(type) === 'refinery' ? 200 : 0, research: buildingRole(type) === 'lab' ? { id: 'vehicleWeapons', progress: .4 } : undefined, reserve: buildingRole(type) === 'capacitor' ? 400 : undefined, lastShot: 1.1 };
  ...
  for (const result of checks.results) {
    assert.equal(result.nonempty, result.role !== 'wall', `${result.type}: operating machinery or storage has a visible indicator`);
    assert.equal(result.changed, result.role !== 'wall', `${result.type}: activity follows simulation time or changed research/storage state`);
    assert(result.frozen, `${result.type}: repeating a paused state is identical`);
    assert(result.powerResponds, `${result.type}: activity dims or slows with insufficient power`);
    assert.equal(result.idleActivity, ['core', 'reactor', 'turret', 'rocketTower', 'capacitor'].includes(result.role), `${result.type}: only appropriate machinery runs while idle`);
