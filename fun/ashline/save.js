...
function validateGame(s) {
  ...
  for (const e of s.entities) {
    ...
    valid(Array.isArray(e.path) && e.path.length <= cells && e.path.every(p=>point(p)&&(p.flock===undefined||typeof p.flock==='boolean')&&(p.trafficId===undefined||integer(p.trafficId,1,s.nextId-1))));
    ...
    if(e.passTargetId!==undefined)valid(e.kind==='unit'&&integer(e.passTargetId,1,s.nextId-1));
    if(e.avoidUntil!==undefined)valid(e.kind==='unit'&&number(e.avoidUntil,0));
    for(const key of ['trafficBlockedAt','crowdPlanAt'])if(e[key]!==undefined)valid(e.kind==='unit'&&number(e[key],0));
    if(e.yieldReturn!==undefined)valid(e.kind==='unit'&&point(e.yieldReturn)&&point(e.yieldPoint)&&integer(e.yieldFor,1,s.nextId-1));
    if(e.yieldWaiting!==undefined)valid(e.kind==='unit'&&typeof e.yieldWaiting==='boolean');
