...
Movement should follow direct open routes and smooth bends instead of a visible tile staircase. Prune unnecessary grid waypoints, use short lookahead to round corners only when the entire swept segment is clear, and ease body heading through the shortest turn. Preserve exact assigned destinations, solid corners, and deterministic save continuation. Combat aiming remains responsive.

Vehicles accelerate and decelerate, limit angular speed, and keep a passing side briefly near traffic. Do not turn the body sharply to follow an emergency collision sidestep. Selected units show individual assigned destination brackets and restrained route segments through currently visible ground. WASD and arrows pan the camera; pointer edges and corners scroll it. Q issues attack move, H stops units. All zoom inputs use five fixed levels with a native-resolution maximum; preserve these controls and snap older saved zoom values on restoration.
