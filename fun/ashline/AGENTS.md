...
Group move and attack-move orders reserve separate reachable destinations with unit-size clearance. Units should visibly settle into their own positions rather than crowd the click point. Preserve sensible nearby assignment and stable slots when orders repeat; keep pathfinding obstacles solid while the existing temporary friendly traffic passing resolves jams.

Movement follows direct open routes, pruning unnecessary grid waypoints while preserving exact destinations, solid corners, and deterministic save continuation. Units rotate in place, then travel straight with a fixed body heading; translation and rotation must never occur in the same simulation tick. Vehicles accelerate and decelerate along each straight leg. Friendly traffic may briefly share space to resolve jams, but must not introduce sideways motion while turning. Combat aiming remains responsive.
